...
def generate_faqs(keyword: str, niche: str) -> list:
    """Generate 4-5 structured FAQ Q&A pairs for FAQPage schema."""
    try:
        client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)
        msg = client.messages.create(
            model="claude-haiku-4-5",
            max_tokens=500,
            messages=[{"role": "user", "content": f'Generate exactly 4 FAQ question-answer pairs about "{keyword}" for a {niche} website. Return ONLY a JSON array: [{{"q":"Question?","a":"Answer in 1-2 sentences."}}, ...]. No preamble, just the JSON array.'}]
        )
        text = msg.content[0].text.strip()
        start = text.find('[')
        end = text.rfind(']') + 1
        if start >= 0 and end > start:
            faqs = json.loads(text[start:end])
            return [f for f in faqs if isinstance(f, dict) and f.get('q') and f.get('a')][:5]
    except Exception:
        pass
    return []
...
"""
    # Inject FAQs into frontmatter for FAQPage JSON-LD schema
    faqs = generate_faqs(keyword, site_config.get("niche", "") if isinstance(site_config, dict) else "")
    if faqs:
        faq_yaml = "faqs:\n"
        for faq in faqs:
            q = str(faq.get("q", "")).replace('"', '\\"').replace("\n", " ").strip()
            a = str(faq.get("a", "")).replace('"', '\\"').replace("\n", " ").strip()
            faq_yaml += f'  - q: "{q}"\n    a: "{a}"\n'
